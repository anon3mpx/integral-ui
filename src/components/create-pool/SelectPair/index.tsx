...
const SelectPair = ({ mintInfo, currencyA, currencyB }: ISelectPair) => {
  ...
  const { onStartPriceInput, onStartPriceInputA } = useMintActionHandlers(
    mintInfo.noLiquidity
  );

  const { startPriceTypedValue, startPriceTypedValueA } = useMintState();
  ...
  const handleTypeInputA = useCallback(
    (value: string) => {
      onStartPriceInputA(value);
    },
    [onStartPriceInputA]
  );
  ...
  return (
    <div className="relative flex flex-col gap-2 items-center">
      <TokenCard
        showBalance={false}
        value={startPriceTypedValueA}
        currency={currencyA}
        otherCurrency={currencyB}
        handleTokenSelection={handleInputSelect}
        handleValueChange={handleTypeInputA}
      />
